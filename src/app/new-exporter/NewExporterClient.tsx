...
export default function NewExporterClient() {
    ...
    <>
      ...
      <SectionWrapper className="bg-[#F8FAFC]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-5xl mx-auto">
          {[
            { num: "01", title: "Not filing LUT before the first export", detail: "Without a Letter of Undertaking (LUT) on the GST portal, you have to pay 18% IGST upfront on your export invoice and claim a refund later — which takes months. File your LUT first and export zero-rated from day one.", color: "red" },
            { num: "02", title: "Wrong HSN code on the Shipping Bill", detail: "Using a broad or incorrect HSN code means your RoDTEP rate, Duty Drawback rate, and certificate requirements will be wrong. A mismatch between invoice HSN and Shipping Bill HSN triggers a customs query.", color: "red" },
            { num: "03", title: "Invoice value doesn't match the Shipping Bill", detail: "Your Commercial Invoice amount and the declared FOB value on the Shipping Bill must match in both amount and currency. Even a rounding difference flags the shipment for examination.", color: "red" },
            { num: "04", title: "Missing AD Code registration at the port", detail: "Your AD Code (bank registration with customs) is port-specific. If you've registered only at JNPT and you ship from Chennai, no Shipping Bill can be filed. Register at every port before you use it.", color: "amber" },
            { num: "05", title: "Not collecting the FIRC", detail: "When your buyer sends payment, your bank issues a FIRC (Foreign Inward Remittance Certificate). This is mandatory for RoDTEP credit, Duty Drawback disbursement, and GST refund claims. Many exporters only realise they need it after the fact.", color: "amber" },
            { num: "06", title: "Exporting restricted goods without checking", detail: "Some products (e.g. certain chemicals, dual-use items, wild flora/fauna) require DGFT licences, SCOMET clearance, or Wildlife Protection Act clearances. Exporting without checking your HSN export policy can result in seizure.", color: "red" },
            { num: "07", title: "Not claiming RoDTEP or Duty Drawback", detail: "This is free money from the government — a remission of taxes embedded in your production cost. Thousands of first-time exporters simply don't know about it and leave it unclaimed. Ask your CHA to file the right Shipping Bill type.", color: "amber" },
            { num: "08", title: "Incoterm on invoice doesn't match the LC terms", detail: "If your buyer opens an LC on CIF terms but your invoice says FOB, the bank will flag a discrepancy and may refuse payment. Always confirm Incoterms with your buyer before generating any documents.", color: "red" },
            { num: "09", title: "Packing List quantity doesn't match physical count", detail: "If your Packing List says 24 cartons of 20 pieces = 480 pieces, but customs physically counts 24 cartons of 18 pieces = 432 pieces, your shipment is held and you face misdeclaration proceedings under the Customs Act.", color: "red" },
            { num: "10", title: "Forgetting to renew the LUT every year", detail: "LUT (Letter of Undertaking) is valid for one financial year (April–March). If it expires and you export without renewing, you have to pay IGST on your exports and claim a refund — tying up working capital for months.", color: "amber" },
          ].map(({ num, title, detail, color }) => (
            <div key={num} className={`bg-white rounded-xl border-l-4 p-5 ${color === "red" ? "border-l-red-500 border border-red-100" : "border-l-amber-500 border border-amber-100"}`}>
              <div className="flex items-start gap-3">
                <span className={`font-bold text-2xl flex-shrink-0 ${color === "red" ? "text-red-300" : "text-amber-300"}`}>{num}</span>
                <div>
        ...
        <div className="mt-8 max-w-5xl mx-auto">
          <div className="bg-[#185FA5]/5 border border-[#185FA5]/20 rounded-xl p-5 text-center">
            ...
            <p className="text-sm text-[#475569]">Our CHA team reviews documents, verifies HSN codes, monitors LUT validity, and tracks all refund claims on your behalf.</p>
          </div>
        </div>
      </SectionWrapper>
